...
if (!cmd||cmd==='--help'||cmd==='-h') {
  ...
  Usage:
    ...
    npx flux-web init --template saas|landing|crud
...
fetch('../pages/home.flux').then(r=>r.text()).then(src=>FLUX.boot(src,document.getElementById('app')))
  ...
  fs.writeFileSync(path.join(dir,'package.json'), JSON.stringify({name,version:'0.1.0',scripts:{dev:'npx flux-web serve',build:'npx flux-web build pages/ --out dist/'},devDependencies:{'flux-web':`^${VERSION}`}},null,2))
...
if (cmd==='build') {
  ...
  console.log(`\n  flux-web build v${VERSION} — ${files.length} file(s)\n`)
  let total=0
  ...
  if(fs.existsSync(hf)){const dst=path.join(outDir,'flux-hydrate.js');fs.copyFileSync(hf,dst);total+=fs.statSync(dst).size;console.log(`  ✓  ${dst.padEnd(40)} ${hSize(fs.statSync(dst).size)}`)}
